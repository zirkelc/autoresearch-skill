...
Two quantities are called "band" here, and they differ by an order of magnitude. The marker uses the **within-run** interquartile range of the per-iteration ratios, which is 5 to 15% on a short case. The per-case bar in step 4 uses the **across-run** spread of that case's medians, which is under 2% for the same case. Keep them apart: the bar must be compared with a quantity measured the way the effect is measured, once per run.

They can therefore disagree, and a campaign hit it: a case at -4.9% and -4.6% in two runs, comfortably above its across-run bar of 3.2%, was flagged in both runs because its iterations were dispersed. The flag does not win. Two runs that agree are the method's decision rule, and a marker is a prompt to look closer, so the resolution is a standalone run on that case, not a discard. A row is no effect when the two runs are marked **and** their medians disagree.

For a short case that is noisy in both senses, this ordering matters more than it sounds: a 1 ms body has a wide within-run band by construction, so a rule that let the marker win would throw away every real effect measured on a small case.
...
Shared code is one of two mechanisms. The other is shared **process state**: libraries keep configuration, registries and caches on `globalThis` on purpose, so that one copy is used however the library is loaded. With two revisions in one process, the instance that initialises last owns that state for both. Measured while testing this harness: two revisions of the same library, one with a changed error message, reported the changed message on both sides. Timing tolerates it, because both sides then run the same configuration, but any comparison of behaviour must run one revision per process. That is why the differential check spawns a process per revision and the standalone check exists at all.
...
- Never quote a single cold timing. Warm up, take the minimum of several timings, and repeat in fresh processes. Two false alarms in one campaign (+6 to +16% and +6 to +19%) were flat once measured warm and repeated, and each cost an afternoon. Treat a first standalone regression as unconfirmed until it repeats.
...
- **Bundle size.** Deterministic, so no noise averaging is needed, but measure gzip (level 9) and brotli (quality 11) bytes. Minified bytes are not what users download: deduplicating repeated code shrinks minified output but is neutral or negative after compression, because compression already encodes repetition almost for free. Only the removal of unique bytes (data, dead code) moves compressed size. A 30-experiment campaign that cut minified size by 2 to 4% per PR had all four PRs closed after the maintainer measured them compressed: one was -1.6 kB minified but +26 B brotli. Gate keep and discard on compressed bytes from the first experiment.
  ...
  Measure the artifact users receive: minified and compressed. An unminified build carries comments and whitespace that no user downloads, and it overstated one change by four times. A package without a bundle step can still be measured by minifying the built file with any minifier and compressing that.

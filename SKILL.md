...
- **Which changes the maintainers will accept.** A change can be correct and faster and still be closed. Read the review comments on the last 5 to 10 performance PRs in the repository, merged and rejected, and write the principles the maintainers state there into the plan. Example from undici: "closely follows the specification, with each step represented explicitly in the code", so a change that merges or skips a spec step is rejected even when its behaviour is identical. Also find the code owner of each area you expect to change (`git log --format='%an' -- <file>`) and their own PRs there, open and closed (`gh pr list --author <owner> --state all`). A change that repeats an idea the owner already tried and dropped will be closed by that owner. Use these principles to rank candidates in step 5, before they cost an experiment.
...
Follow `references/pr-packaging.md`. In short: review the kept commits, search open and closed PRs for earlier attempts at each change and check it against the maintainers' principles, group them into a few independent PRs by theme, cherry-pick each group onto a fresh base branch in its own worktree, verify each branch alone (tests, guard, a noise-control run, two A/B runs against the base, and a standalone run for each headline number), and write each PR body in the repository's own PR template.

**Keep the PR body short and factual.** Fill in the repository's PR template first. Write in ASD-STE100 Simplified Technical English. By default the body says what changed, why it is faster, one number for the targeted workload, and which tests ran. Add the campaign and methodology preamble, verification tables, reproduction commands or a harness link only if the user asks for them. `references/pr-packaging.md` section 5 has the rules.
...
**Every number in a PR comes from a standalone run, measured next to an identical-code control.** The reason is not that paired numbers are inflated (they are sometimes lower), it is that a maintainer builds one revision per process, so the standalone number is the one they will get. The control is what makes that number evidence: process-to-process spread differs by a factor of twenty between cases. An effect counts as resolvable when its median lies outside the control's range in two runs made on different occasions; otherwise report the focused number, name the instrument, and say the control could not separate it. One campaign published a 1.17x claim for a case whose control later turned out to span -19% to +18%, and had to correct the PR.

**Measure the whole user-facing operation before any sentence implies that it got faster.** A case measures one function. In one campaign, four PRs each made their function measurably faster, and all four together changed a complete `fetch()` call by -1.5%, which was inside the noise. Say what the change makes faster, and give the effect on the whole operation next to it.
...
Show the user each PR (title, branch, commits, verification table, body and its length in lines) and wait for confirmation before you push or create it. Create each PR only after its confirmation. Follow the target repo's own PR conventions where they exist.
